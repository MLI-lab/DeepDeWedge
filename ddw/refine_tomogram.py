...
from .utils.missing_wedge import get_missing_wedge_mask, apply_fourier_mask_to_tomo
...
loader = lambda yaml_config_file: load_function_args_from_yaml_config(function=refine_tomogram, yaml_config_file=yaml_config_file)
...
def refine_tomogram(
    ...
    model_checkpoint_file: Annotated[Path, typer.Option(help="Path to a model checkpoint file (.ckpt extension). Checkpoints saved during model fitting can be found in the 'logdir' directory specifed for the 'fit-model' command.")],
    subtomo_size: Annotated[int, typer.Option(help="Size of the cubic subtomograms to extract. This should be the same as the subtomo_size used during model fitting.")],
    mw_angle: Annotated[int, typer.Option(help="Width of the missing wedge in degrees.")],
    subtomo_overlap: Annotated[Optional[int], typer.Option(help="Overlap between subtomograms. This determines the stride of the sliding window used to extract subtomograms. If 'None', this is set to 1/3 * subtomo_size.")] = None,
    recompute_normalization: Annotated[bool, typer.Option(help="Whether to recompute the mean and variance used to normalize the tomo0s and tomo1s. If `False`, the mean and variance of model inputs calculated during model fitting will be used. If `True`, the average model input mean and variance will be computed for each tomogram individually. We recommend setting this to to `True`. If you apply a model to a tomogram that was not used for model fitting or if the means and variances of the tomograms during model fitting are considerably different, recomputing the normalization is expected to be very beneficial for tomogram refinement.")] = True,
    batch_size: Annotated[int, typer.Option(help="Batch size for processing subtomograms.")] = 1,
    return_tomos: Annotated[bool, typer.Option(help="Whether to return the refined tomograms as a list of numpy arrays. If False, the refined tomograms will only be saved to the output_dir.")] = False,
    output_dir: Annotated[Optional[Path], typer.Option(help="Where to save the refined tomograms. If not provided, output dir is set to project_dir/refined_tomograms.")] = None,
    project_dir: Annotated[Optional[Path], typer.Option(help="Path to the project directory. If return_subtomos is False, and output_dir is not provided, this has to be provided.")] = None,
    num_workers: Annotated[int, typer.Option(help="Number of CPU workers to use during the recomputation of the normalization statistics and dataloading for refining the tomograms.")] = 0,
    gpu: Annotated[Optional[int], typer.Option(help="GPU id on which to run the model. If None, the model will be run on the CPU.")] = None,
    ...
    if output_dir is None:
        if project_dir is not None:
            ...
            raise ValueError("If return_tomos is False, output_dir or project_dir must be provided, otherwise the refined tomograms will be lost.")
    ...
    lightning_model = LitUnet3D.load_from_checkpoint(model_checkpoint_file).to(device).eval()
    ...
    with torch.no_grad():
        for t0_file, t1_file in zip(tomo0_files, tomo1_files):
            if recompute_normalization:
                ...
                loc, scale = lightning_model.unet.normalization_loc.clone().detach().item(), lightning_model.unet.normalization_scale.clone().detach().item()
            print(f"######### {loc, scale}")
