...
def load_function_args_from_yaml_config(function, yaml_config_file, shared_field_name="shared"):
    ...
    if function.__name__ not in cfg.keys():
        raise ValueError(f"Field '{function.__name__}' not found in config file '{yaml_config_file}'!")
    ...
    for arg in inspect.getfullargspec(function).args:
        ...
        if arg in cfg.keys():
            if found_in_common:
                print(f"Overriding value '{args[arg]}' for argument '{arg}' found in field '{shared_field_name}' with value '{cfg[arg]}' from field '{function.__name__}'!")
            args[arg] = cfg[arg]
    return args
